...
export default function VideoPage(){
    ...
    return (
        <div className="bg-[#E8E2F4]">
            <div className="h-32 w-full  flex flex-row gap-9 justify-center items-center">
                {
                            ...
                            <div className="flex flex-row gap-3">
                               {
                                 renderSlideDiv.map((slideitem,slideindex) =>(
                                   <div className={`w-8 h-1 rounded-md ${index*6 + slideindex + 1 <= page ? 'bg-[#000]' : 'bg-[#fff]'}`}></div>
                                 ))
                               }
                            </div>
                            <div className="flex flex-row gap-3">
                                 {
                                   renderQnDiv.map((qnitems,qnindex)=>(
                                     <div className="w-8 h-1 bg-[#8C52FF] rounded-md"></div>
                                   ))
                                 }
                            </div>
                        </div>
            ...
            <div className="h-52 flex justify-between items-center ml-8">
                    <div className="flex flex-row gap-6">
                        <div><Image src="/play.svg" alt="play" height={100} width={100}/></div>
                        <div className="flex flex-col justify-center">
                            <span className="text-3xl font-medium">Data Analytics</span>
                            <span className="text-xl font-medium text-[#737373]">BYTE {page}/24</span>
                        </div>
                    </div>
                    <div className="w-24 h-24 bg-[#ffffff] rounded-full mr-8 flex justify-center items-center"><Image src="/robot icon.svg" alt="bot" height={50} width={50}/></div>
            </div>
        </div>
    );
}