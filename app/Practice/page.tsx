              ...
              <div className="grid grid-cols-2  gap-4 ">
                {Alldata.map((item, index) => (
                  <>
                    <div
                      key={item.id}
                      className="bg-[#ffffff] rounded-lg flex flex-row justify-between h-32"
                    >
                      <div className="w-1/2  ml-4 flex flex-col gap-y-6 justify-center text-base font-semibold ">
                        {item.name}
                        <button
                          className="w-20 bg-[#8c52ff] rounded-2xl flex flex-row justify-between items-center shadow shadow-[#737373]"
                          onClick={toggleAsmtVisible}
                        >
                          <span className="ml-2 text-[#ffffff] font-medium">
                            Start
                          </span>
                          <Image
                            src="/practice/conarrow.svg"
                            alt="arrow"
                            width={25}
                            height={25}
                          />
                    ...
                    {index % 2 === 1 && (
                      <div
                        className={`col-span-2 w-full bg-[#fff] flex justify-center rounded-lg ${
                          asmtVisible ? "flex" : "hidden"
                        }`}
                      >
                        <div className="w-10/12 flex flex-col gap-3 mt-6 mb-6 ">
                          {codingdata.map((item, index) => (
                            <div
                              className="flex flex-row w-full h-10 gap-8"
                              key={index}
                            >
                              <div className="flex flex-row w-9/12 h-full bg-[#F1EAFF] rounded-full gap-4 items-center">
                                <div>
                                  <img src={codingdata[index].svg} />
                                </div>
                                <div className="text-[#737373] text-md font-semibold">
                                  {codingdata[index].name}
                                </div>
                              </div>
                              <button className="w-4/12 h-10 border-2 border-[#8C52FF] rounded-full text-[#2561A1] font-semibold flex justify-center items-center shadow-lg">
                                Start Now
                              </button>
                            </div>
                          ))}
        ...
        <div className="w-full max-w-[800px]  flex flex-col gap-2 p-3 mt-1 lg:mt-0">
          <div className="h-1/2 flex flex-col m-2 mt-3 bg-[#ffffff] rounded-xl">
            ...
            <div className="flex flex-row justify-around h-1/2 m-5 gap-4">
              <div className="h-32 border-2 rounded-md border-[#ebedf1] w-full text-lg font-semibold">
                <span className="ml-2 mt-2">Badges</span>
              </div>
              <div className="h-32 border-2 rounded-md border-[#ebedf1] w-full text-lg font-semibold">
                <span className="ml-2 mt-2">Focused Area</span>
              </div>
            </div>
          </div>
          <div className="bg-[#ffffff] h-1/2 m-3  flex flex-col gap-y-3 rounded-xl pb-4">
            <div className="flex flex-row justify-around h-1/5">
              ...
              <div className="flex flex-row mt-3">
                <div className="w-9 h-9 rounded-full border-[#F1EAFF] border-2 flex items-center justify-center">
                  <Image
                    src="/lfarw.svg"
                    alt="lft"
                    width={7.89}
            ...
            <div className="grid grid-cols-3 place-items-center">
              {items.map((items, itemindex) => (
                <div
                  className="container grid grid-cols-5 gap-2 place-content-center w-24"
                  key={itemindex}
                >
                  {sets.map((item, setindex) => (
                    <div
                      className="h-4 w-4 bg-[#ebedf1] rounded "
                      key={setindex}
                    ></div>
                  ))}
                </div>
            ...
            <div className="h-1/5 flex flex-col gap-4">
              <div className="flex flex-row gap-4 justify-center mt-3">
                ...
                <div className="flex flex-row gap-3">
                  <div className="flex justify-center items-center border-2 rounded border-[#dadce0] w-6 h-6 ">
                    <FaStar style={{ color: "rgb(3, 193, 101)" }} />
                  </div>
